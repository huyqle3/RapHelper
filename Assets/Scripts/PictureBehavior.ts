...
export class PictureBehavior extends BaseScriptComponent {
  ...
  private loadCaption(text: string) {
    ...
    // Add vertical and rightward offset (adjust as needed)
    var upOffset = this.picAnchorTrans.up.uniformScale(1);     // 1 cm above
    var rightOffset = this.picAnchorTrans.right.uniformScale(25); // 0.5 cm to the right
  
    var captionPos = topCenterPos.add(upOffset).add(rightOffset);
